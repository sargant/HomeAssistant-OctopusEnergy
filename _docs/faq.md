...
!!! info
    Electricity and gas meter entity IDs are now generated by Home Assistant. The documented IDs are reproduced when your entity naming scheme is `Device Entity`, with the meter identity supplied by the device name and the entity-specific part supplied by the entity name. Existing registered entity IDs are retained automatically. See [Entity IDs and names](./entity_ids.md) for further details.
